...
<svelte:head>
	<title>{`${$t('point-history.head.title')} ${PAGE_TITLE_POSTFIX}`}</title>
</svelte:head>
				...
				<div class="tabs-contents _bgcl-white-100">
					...
					>
						<table class="trb-table">
							<thead>
								<tr>
									<th>{$t('point-history.table-column.redeemed-date')}</th>
							...
							<tbody class="_bgcl-white-100">
								{#each list as it}
									<tr>
										...
										<td>{it.name}</td>
										<td class="_tal-r _pdr-32px _fw-600 _cl-negative-200">
						...
						{#each list as it, idx}
							<div class="_dp-f _jtfct-spbtw _gg-24px _pd-16px">
								...
								<div style="padding-top: 18px">
									<div class="_fw-500 _fs-400 _tal-r _cl-negative-200 _wsp-nw">
										{format.point(it.points)}
									</div>
								</div>
							</div>
